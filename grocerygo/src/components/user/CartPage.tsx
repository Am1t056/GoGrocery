    ...
    <div className="w-[95%] sm:w-[80%] mx-auto mt-8 mb-24 relative">
      ...
      {cartData.length === 0 ? (
        ...
        >
          ...
          <Link
            href={'/'}
            className="text-green-600 hover:underline underline-offset-2 hover:text-green-700 transition-all  font-medium flex items-center gap-2 text-center justify-center text-sm"
          >
            Continue Shopping <MoveRight className="size-5" />
          </Link>
        </motion.div>
      ) : (
            ...
            <AnimatePresence>
              {cartData.map((item, index) => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -20 }}
                  className="flex flex-col sm:flex-row items-center bg-white rounded-2xl shadow-md p-5 hover:shadow-2xl transition-all duration-300 border border-gray-300"
                >
                  <div className="relative w-28 h-28 sm:w-24 sm:h-24 md:h-28 shrink-0 rounded-xl overflow-hidden bg-gray-50">
                    <Image
                      src={item.image}
                      alt={item.name}
                      fill
